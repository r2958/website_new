...
def main():
    ...
    field_dims = [
        len(user_encoder.classes_),      # user_id 维度
        len(product_encoder.classes_),   # product_id 维度
        24,                               # hour (0-23)
        7,                                # day_of_week (0-6)
        5                                 # price_bucket (0-4)
    ]
    ...
    model = train_model(model, train_loader, val_loader, epochs=100, lr=0.001, device=device)
    ...
    with torch.no_grad():
        # 取前5个样本测试
        test_X = torch.LongTensor(X_val.head(5).values).to(device)
        predictions = model(test_X).cpu().numpy()
        
        print("\n预测结果示例:")
        for i, (idx, row) in enumerate(X_val.head(5).iterrows()):
            user_id = user_encoder.inverse_transform([row['user_idx']])[0]
            product_id = product_encoder.inverse_transform([row['product_idx']])[0]
            true_label = y_val.iloc[i]
            pred_score = predictions[i]
            
            print(f"  用户: {user_id[:20]}... | 商品: {product_id} | "
                  f"真实: {true_label} | 预测: {pred_score:.4f}")
