...
if(isset($_POST['done']) && ($_POST['done'] == 'Yes')) {
	...
	$Admin->doUpdateStateTaxes();
	include($CFG->serverroot . '/common/admin_area/inc_liveupdate.php');
	die;
}

$qid = $DB->query("SELECT * FROM taxes_states ORDER BY State");
$count = 0;

$Page->PageTitle = 'State Tax Rates';
...
<form name="FormName" method="post" action="<? echo $_SERVER['PHP_SELF']; ?>" onsubmit="sendPost(this.name); return false;">
	...
	<table border="0" cellpadding="2" cellspacing="0" class="sortable">
		<tr>
			<th valign="bottom" nowrap="nowrap">State</th>
		...
		<tr>
			<td><? $ShoppingCart->pv($row->State) ?><input type="hidden" value="<? $ShoppingCart->pv($row->State) ?>" name="State[]"></td>
